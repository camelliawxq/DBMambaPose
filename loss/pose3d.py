...
def mean_velocity_error(predicted, target, axis=0): # mixSTE
    """
...
def weighted_mpjpe(predicted, target, w):
    ...
    """
    assert predicted.shape == target.shape
    assert w.shape[0] == predicted.shape[2]  # torch.Size([17])  torch.Size([24, 243, 17, 3])
...
def loss_TCL(predicted, w_mpjpe):
    # Temporal Consistency Loss
    dif_seq = predicted[:, 1:, :, :] - predicted[:, :-1, :, :]  # 后一帧-前一帧 (b, f-1, n, c)
