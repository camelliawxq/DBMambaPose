...
def load_model(args, adj=None, adj_temporal=None):
    act_mapper = {
        "gelu": nn.GELU,
        'relu': nn.ReLU
    }
    ...
    if not(hasattr(args, "model_name")):
        args.model_name = 'DSTformer' # Default
    if args.model_name=='DSTformer':
        model = DSTformer(dim_in=3, dim_out=3, dim_feat=args.dim_feat, dim_rep=args.dim_rep,
                                   depth=args.n_layers, num_heads=args.num_heads, mlp_ratio=args.mlp_ratio, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                                   maxlen=args.n_frames, num_joints=args.num_joints)

    elif args.model_name == "DBMambaPose":
        from model.DBMambaPose import DBMambaPose
        model = DBMambaPose(n_layers=args.n_layers,
                               dim_in=args.dim_in,
    ...
    elif args.model_name == "MotionAGFormer":
