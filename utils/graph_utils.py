...
def adj_mx_from_edges(num_pts, edges, sparse=True):
    ...
    data, i, j = np.ones(edges.shape[0]), edges[:, 0], edges[:, 1]  # 直接相连的边为1，其余为0
    adj_mx = sp.coo_matrix((data, (i, j)), shape=(num_pts, num_pts), dtype=np.float32)

    # build symmetric adjacency matrix  与布尔矩阵相乘，只保留需要更新的位置的值
    ...
    adj_mx = adj_mx * (1-torch.eye(adj_mx.shape[0])) + torch.eye(adj_mx.shape[0])  # 使对角线上元素为1，添加自环
...
def adj_mx_from_skeleton_temporal(num_frame, parents):
    ...
    edges = list(filter(lambda x: x[1] >= 0, zip(list(range(0, num_joints)), parents)))  # [(1, 0), ..., (子节点, 父节点)]
    return adj_mx_from_edges(num_joints, edges, sparse=False)
