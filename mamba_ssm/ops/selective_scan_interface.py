...
class SelectiveScanFn(torch.autograd.Function):
    ...
    def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
        ...
        if C.dim() == 3:
            ...
            ctx.squeeze_C = True
        # print(u.shape, delta.shape, A.shape, B.shape, C.shape, D.shape)
...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    dtype_in = u.dtype
    u = u.float()
    delta = delta.float()  # (B D dstate)
    if delta_bias is not None:  # False
    ...
    if A.is_complex():  # False
        ...
        B = B.float()  # (B, dstate, L)
        C = C.float()  # (B, dstate, L)

    # x = A.new_zeros((batch, dim, dstate)) if h0==None else h0

    ys = []
    deltaA = torch.exp(torch.einsum('bdl,dn->bdln', delta, A))  # (A-) (B,D,L,dstate)
    if not is_variable_B:  # False
        deltaB_u = torch.einsum('bdl,dn,bdl->bdln', delta, B, u)
    else:
        if B.dim() == 3:  # True
            deltaB_u = torch.einsum('bdl,bnl,bdl->bdln', delta, B, u)  # (B- * x_t) (B,D,L,dstate)
        else:
            B = repeat(B, "B G N L -> B (G H) N L", H=dim // B.shape[1])
            deltaB_u = torch.einsum('bdl,bdnl,bdl->bdln', delta, B, u)
    if is_variable_C and C.dim() == 4:  # False
        C = repeat(C, "B G N L -> B (G H) N L", H=dim // C.shape[1])
    last_state = None
    # 均值
    x = deltaB_u.clone().mean(dim=-2)  # (B D dstate) 初始化隐藏状态 h_(t-1)

    for i in range(u.shape[2]):  # seq_len
        x = deltaA[:, :, i] * x + deltaB_u[:, :, i]  # h_t = A- * h_(t-1) + B- * x_t  (B D dstate)
        if not is_variable_C:  # False
            y = torch.einsum('bdn,dn->bd', x, C)
        else:
            if C.dim() == 3:  # True
                y = torch.einsum('bdn,bn->bd', x, C[:, :, i])  # y=C*h_t (B,D)
            else:
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        conv1d_weight = rearrange(conv1d_weight, "d 1 w -> d w")  # (dim*expand, 2)
        x, z = xz.chunk(2, dim=1)  # (batch, dim*expand, seqlen)
        conv1d_bias = conv1d_bias.contiguous() if conv1d_bias is not None else None
        conv1d_out = causal_conv1d_cuda.causal_conv1d_fwd(x, conv1d_weight, conv1d_bias, None,
                                                          True)  # (batch, dim*expand, seqlen)
        # We're being very careful here about the layout, to avoid extra transposes.
        # We want delta to have d as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
        x_dbl = F.linear(rearrange(conv1d_out, 'b d l -> (b l) d'), x_proj_weight)  # (bl d)
        delta = rearrange(delta_proj_weight @ x_dbl[:, :delta_rank].t(), "d (b l) -> b d l",
                          l=L)  # (batch, dim*expand, seqlen)
        ...
        if not A.is_complex():
            B = rearrange(B, "(b l) dstate -> b 1 dstate l", l=L).contiguous()  # (512, 1, 4, 17)
        else:
            B = rearrange(B, "(b l) (dstate two) -> b 1 dstate (l two)", l=L, two=2).contiguous()
        # variable C
        C += x_dbl[:, -d_state:]  # (bl dstate)
        if not A.is_complex():
            C = rearrange(C, "(b l) dstate -> b 1 dstate l", l=L).contiguous()  # (512, 1, 4, 17)
        else:
...
class MambaInnerFnNoOutProj_learnable(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        """
             xz: (batch, dim, seqlen)
        """
        # B1, C1, delta1 = B.float(), C.float(), delta.float()
        assert checkpoint_lvl in [0, 1]
        L = xz.shape[-1]  # 17
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
        ...
        x_dbl = F.linear(rearrange(conv1d_out, 'b d l -> (b l) d'), x_proj_weight)  # (bl d)
        delta = delta.float()
        # delta = rearrange(delta_proj_weight @ x_dbl[:, :delta_rank].t(), "d (b l) -> b d l", l = L) # (batch, dim*expand, seqlen)
        # delta = alpha[2] * delta + (1 - alpha[2]) * delta1
        ctx.is_variable_B = True
        ctx.is_variable_C = True
        ctx.B_proj_bias_is_None = True
        ctx.C_proj_bias_is_None = True
        # variable B
        B = B.float().view(-1, d_state)
        # B = x_dbl[:, delta_rank:delta_rank + d_state]  # (bl dstate)
        # B = alpha[0] * B + (1 - alpha[0]) * B1.view(-1, d_state) # (bl dstate)
        if not A.is_complex():
            B = rearrange(B, "(b l) dstate -> b 1 dstate l", l=L).contiguous()  # (512, 1, 4, 17)
        else:
            B = rearrange(B, "(b l) (dstate two) -> b 1 dstate (l two)", l=L, two=2).contiguous()
        # variable C
        C = C.float().view(-1, d_state)
        # C = x_dbl[:, -d_state:]  # (bl dstate)
        # C = alpha[1] * C + (1 - alpha[1]) * C1.view(-1, d_state)
        if not A.is_complex():
            C = rearrange(C, "(b l) dstate -> b 1 dstate l", l=L).contiguous()  # (512, 1, 4, 17)
        else:
            C = rearrange(C, "(b l) (dstate two) -> b 1 dstate (l two)", l=L, two=2).contiguous()
...
def mamba_inner_fn_no_out_proj_learnable(
        xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        A, B=None, C=None, D=None, delta_bias=None, B_proj_bias=None,
        C_proj_bias=None, delta_softplus=True, delta=None, alpha=None
):
    return MambaInnerFnNoOutProj_learnable.apply(xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
                                                 A, B, C, D, delta_bias, B_proj_bias,
                                                 C_proj_bias, delta_softplus, 1,
                                                 delta, alpha)
