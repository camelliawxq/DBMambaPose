...
class MPI3DHP(Dataset):
    ...
    def prepare_data(self, args):
        ...
        p = os.path.join(args.data_root, data_file)
    ...
    def get_vector(self, keypoints, cam=None, add_norm=True):
        ...
        for i in range(keypoints.shape[0]):
            ...
            if add_norm:
                ...
                cos_ODC = - np.sum(OD * DC, axis=1) / (
                            (np.linalg.norm(OD, axis=1) + 1e-4) * (np.linalg.norm(DC, axis=1) + 1e-4))
                cos_ODC[i] = 0
